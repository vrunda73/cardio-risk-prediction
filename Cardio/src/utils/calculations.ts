...
export function getBPCategory(systolic: number, diastolic: number) {
  ...
  return {
    label: 'High — Stage 2',
    color: '#ef4444',
    description: 'Seek medical advice',
    severity: 3,
  };
}
...
export function validateAssessmentStep(
  ...
  if (step === 2) {
    ...
    // Blood pressure:
    // NO minimum or maximum range restriction.
    if (
      data.ap_hi === undefined ||
      data.ap_hi === null ||
      Number.isNaN(Number(data.ap_hi))
    ) {
    ...
    if (
      ...
      data.ap_lo === null ||
      Number.isNaN(Number(data.ap_lo))
    ) {
      errors.push('Please enter a valid diastolic blood pressure.');
    }

    // No requirement that diastolic must be lower than systolic.
    // This allows any numeric blood-pressure values.
  }
